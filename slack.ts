...
export async function postMessage(submission: DecryptedContentAndAttachments, requestBody) {
    const formId = getFormId(requestBody);
    const submissionUrl = `${FORMSG_PREFIX}/admin/form/${formId}/results/${(getSubmissionId(requestBody))}`;

    const formFields = submission.content.responses
        .filter((formField) => {
            const whitelistExists = Object.keys(FORM_WHITELISTED_FIELD_IDS).includes(formId);
...
export async function uploadJsonFile(submission: DecryptedContentAndAttachments, requestBody) {
    const simplifiedResponse = Object.assign({}, ...submission.content.responses.map(formField => {
        return {
...
function resolveChannelId(formId: string): string {
    return SLACK_BOT_CHANNEL_ID_MAPPING[formId] || SLACK_BOT_CHANNEL_ID_MAPPING["default"];
}