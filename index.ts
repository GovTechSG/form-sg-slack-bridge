...
async function handleSubmissions(event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> {
    const signature = event.headers['x-formsg-signature'] || event.headers['X-FormSG-Signature'];
    ...
    for (const formSecretKey of FORM_SECRET_KEYS) {
        const submission = await formsgSdk.crypto.decryptWithAttachments(formSecretKey, requestBody.data);
